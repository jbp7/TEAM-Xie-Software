...
<p align="justify">
The accompanying dataset `EQAPOL_Ex` is a sample from the External Quality Assurance Program Oversight Laboratory proficiency program by the Duke Immune Profiling Core. The dataset contains the results of a flow-cytometry experiment conducted on a blood sample of a single healthy individual. The blood sample was split into two parts: one was used as a negative control (cohort 1) and the other was stimulated with a peptide mixture from the immunodominant cytomegalovirus (CMV) pp65 protein (cohort 2). Results for each cohort are summarized as data frames labeled `Costim` for cohort 1 and `CMV` for cohort 2. Each data frame contains expression data on approximately 200 thousand cell observations (rows) and 11 protein markers (columns). One can attach the dataset by calling:
 </p>
...
ggpairs(subj_eqapol, columns=1:4, 
...
TEAM is designed to identify the differential density regions or areas where activated cells are enriched. However, it cannot directly phenotype the functionality of individual, activated cells. On the other hand, we may estimate the phenotype of certain enriched sets comprised of these activated cells. The main idea is to utilize the subspaces spanned by key protein markers. Specifically, we create subspaces based on pairwise combinations of markers, apply TEAM to these combinations, and identify differential density regions for each combination. We can then take intersections over all identified regions to define three types of enriched cell sets: monofunctional, bifunctional, and polyfunctional. 
